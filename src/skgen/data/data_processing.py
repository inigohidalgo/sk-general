...
def _is_true(
    series: pd.Series, true_key: Any = "Y", false_key: Optional[Any] = None
) -> pd.Series:
    """
    ...
    **contains_kwargs: Dict[str, Any],
    ...
    if isinstance(axis, str):
        if axis == "columns":
            ...
            raise ValueError(
                "If passed as a str, axis must be one of 'columns' or 'index'"
            )
...
# or reincorporate dispatch into handler
@dataclass
class DataConversionDispatch:
    converters: Dict[Tuple[str, str], Callable] = field(
        default_factory=get_base_converters
    )
    transformers: Iterable[Callable] = field(default_factory=list)
    verifiers: Dict[str, Callable[[Any], bool]] = field(
        default_factory=get_base_verifiers
    )
...
class DataHandler:
    ...
    def __init__(
        ...
        self.data_status = DataStatus(
            input_type, output_type, processing_type, input_type
        )
        self.add_operations(
            converters=converters, verifiers=verifiers, transformers=transformers
        )
    ...
    def convert_data(self, data, input_type, output_type, **conversion_kwargs):
        ...
        if all(
            conversion_keys
        ):  # if either input or output type is None, we skip conversion
            return self.operations.converters[conversion_keys](
                data, **conversion_kwargs
            )
        else:
            return data
    ...
    def receive_data(self, data) -> None:
        if self.verify_data_type(data, self.data_status.input_type):
            ...
            self.data = self.convert_data(
                self.data,
                self.data_status.current_type,
                self.data_status.processing_type,
            )
